		...
		<form method="post" id="permissions" class="validate" autocomplete="off" action="{{ url('permission/store') }}">
			...
			<div class="card">
				...
				<div class="card-body"> 
					...
					<hr>
										<b class="text-muted mb-3 d-block">{{ _lang('Permisos:') }}</b>
                                         <!--   <div class="custom-control custom-checkbox">
                                                <input type="checkbox" class="custom-control-input" id="select-all">
                                                <label class="custom-control-label" for="select-all">{{ _lang('Seleccionar todos') }}</label>
                                            </div>-->
					...
					<div id="accordion">
					@php $i = 1; @endphp
					@foreach ($permisos as $outerItem)
					<div class="card">
						<div class="card-header">
						  <h4>
							  <a class="card-link" data-toggle="collapse" href="#collapse-{{ str_replace(" ","",$outerItem['grupo']) }}">
								<i class="fa fa-angle-double-right" aria-hidden="true"></i>
								 {{ $outerItem['grupo'] }}
							  </a>
						  </h4>
						</div>
						<div id="collapse-{{ str_replace(" ","",$outerItem['grupo']) }}" class="collapse">
						  <div class="card-body">
							  <table class="table">
								@foreach ($outerItem['items'] as $innerItem)	
									<tr>
										<td>
											<div class="checkbox">
												<div class="custom-control custom-checkbox">
													<input type="checkbox" class="custom-control-input" name="permissions[]" value="{{ $innerItem }}" id="customCheck{{ $i + 1 }}" {{ array_search($innerItem,$permission_list) !== FALSE ? "checked" : "" }}>
													<label class="custom-control-label" for="customCheck{{ $i + 1 }}">{{ $innerItem }}</label>
												</div>
											</div>
										</td>
									</tr>
