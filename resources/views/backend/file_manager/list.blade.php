...
<div class="row">
	...
	<div class="col-12">
		<h3 class="my-2">Archivos de {{ $parent->name ?? '' }} @if($dominio != '') Dominio: {{ $dominio }} @endif</h3>
		...
		<a class="btn btn-primary btn-xs ajax-modal" data-title="{{ _lang('Carga Multiple') }}" href="{{ url('file_manager/create-multiple/'.request()->route('parent_id')) }}"><i class="fas fa-folder-plus"></i> {{ _lang('Carga Multiple') }}</a>
					    ...
					    <div class="dropdown">
							...
							<div class="dropdown-menu">
								...
								<form action="{{ action('FileManagerController@destroy', $filemanager['id']) }}" method="post">									
									...
									<?php $os = array("tramitador","Tramitador con gastos");
								if (!in_array(strtolower(auth()->user()->role->name), $os)) {
									echo '<button class="button-link btn-remove" type="submit"><i class="fas fa-trash-alt"></i>'. _lang('Delete').'</button>';
									}?>
								</form>
