				...
				<form method="post" class="validate" autocomplete="off" action="{{ action('SalesReturnController@update', $id) }}" enctype="multipart/form-data">
					<div class="row">  
						...
						<div class="col-md-4">
							...
							<input type="date" class="form-control datepicker" name="return_date" value="{{ $sales->return_date }}" readOnly="true" required>
							</div>
						</div>
						...
						<div class="col-md-4">
						  <div class="form-group">
							...
							<select class="form-control select2-ajax" data-value="id" data-display="contact_name" data-table="contacts" data-where="1" name="customer_id" id="customer_id" required>
								 ...
								 {{ create_option("contacts","id","contact_name",$sales->customer_id) }}
							</select>	
						  </div>
						</div>
								...
								<table id="order-table" class="table table-bordered">
									...
									<tbody>
										@foreach($sales->sales_return_items as $item)
											<tr id="product-{{ $item->product_id }}">
												<td>
													<b>{{ "(" . (($item->company_id == 1) ? 'PM-' : 'PC-') . $item->product_id . ") "   .  $item->item->item_name }}</b><br>
												</td>
												...
												<td class="text-center quantity">1 <input class="form-control input-quantity" type="hidden" value="1" name="quantity[]"></td>
												...
												<input type="hidden" name="sales_return_items_id[]" value="{{ $item->id }}">
									...
									<tfoot class="tfoot active">
										<tr>
												...
												<th class="text-right" id="product_sub_total">0.00</th>
