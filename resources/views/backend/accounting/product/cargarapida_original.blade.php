                ...
                <div class="card-body">
                        ...
                        <thead>
                            <tr>
                                ...
                                <th class="text-right" style="width: 300px;">{{ _lang('Marca y modelo') }}</th>
                                <th class="text-right" style="width: 300px;">{{ _lang('nº oblea') }}</th>
                                <th class="text-right" style="width: 100px;">{{ _lang('nº motor') }}</th>
                        ...
                        <tbody>
                            ...
                            @foreach ($products as $product)
                                ...
                                <tr id="row_{{ $product->id }}">
                                    ...
                                    <td class='product_cost text-right'>
                                        {{ ($product->marcaModelo->marca->marca ?? '') . ' ' . ($product->marcaModelo->modelo->modelo ?? '') }}
                                    </td>
                                    <td>{{ $product->nro_oblea }}</td>
                                    <td>{{ $product->nro_motor }}</td>
                                    <td>{{ $product->deposito->nombre }}</td>
                                    <td>{{ $product->ubicacion }}</td>
                                    {{-- <td class='product_price text-right'>{{ decimalPlace($product->product->product_price, $currency) }}</td> --}}
                                    {{-- <td class='product_unit'>{{ $product->product->product_unit }}</td> --}}
                                    <td>{{ $product->description }}</td>
                                    <td>{{ $product->created_at->format('d/m/Y H:i') }}</td>
                                    <td>{{ $product->user->name }}</td>
                                    <?php 
                                        $result=  "<form action='". action('ProductController@destroy', $product->id) ."' method='post'>";
                                        $result .= "<a href='" . action('ProductController@show', $product->id) . "' class='btn btn-primary btn-xs ajax-modal'><i class='ti-eye'></i></a>";

                                        $result .= "<a href='" . action('ProductController@printQR', $product->id) . "' class='btn btn-success btn-xs ajax-modal'><i class='fa fa-qrcode' aria-hidden='true'></i></a>";

                                        $result .= "<a href='" . action('ProductController@printsinQR', $product->id) . "' class='btn btn-success btn-xs ajax-modal'><i class='fa fa-barcode' aria-hidden='true'></i></a>";
                                        $result .= csrf_field();
                                          $result .= "</form>";
                                          echo "<td>$result </td>";
                                      ?>
