								...
								<table id="order-table" class="table table-bordered">
									...
									<?php $q_anuladosr =(isset($anulados) && $anulados!="" ) ? explode(",", $anulados):array();	?>
										...
										{{-- {{ dd($item->product->marcaModelo->marca->marca) }} --}}
											<tr <?php echo (in_array($item->product_id,$q_anuladosr)) ? 'class="row-disabled"' :''; ?> id="product-{{ $item->product_id }}"  data-id="{{ $item->id }}">
											<td><input name="bank_check" type="checkbox" class="fila-seleccionada" data-id="{{ $item->id }}"></td>
												<td>
												...
												<td class="text-center quantity">1 <input type="hidden" value="1" name="quantity[]" min="1" class="form-control input-quantity text-center" value="{{ $item->quantity }}" max="{{ 1 + $item->item->product->stock }}"></td>
												...
												<td class="text-right usd"><input disabled id="usd_monto-{{ $item->product_id }}" type="text" class="form-control input-usd text-right" ></td>
	...
	<div class="modal-dialog" role="document">
	  <div class="modal-content">
		...
		<div class="modal-footer">
		  <button type="button" id="product_eliminar" class="btn btn-primary">Continuar</button>
