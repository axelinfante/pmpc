                                            ...
                                            <tbody>
                                                <tr>
                                                    <td>
                                                        ...
                                                        {!! get_company_option('vat_id') != ''
                                                            ? _lang('VAT ID') . ': ' . clean(get_company_option('vat_id')) . '<br>'
                                                            : '' !!}
                                                        {!! get_company_option('reg_no') != ''
                                                            ? _lang('REG NO') . ': ' . clean(get_company_option('reg_no')) . '<br>'
                                                            : '' !!}
                                                    </td>
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="invoice-col-6 pt-3">
                                                <div class="d-inline-block float-md-right">
                                                    <h5><b>{{ _lang('Cotización') }} #:</b> {{ $invoice->invoice_number }}
                                                    </h5>
                                                    ...
                                                    <!--<b>Observaciones de desarme:</b>
                                                    @forelse ($desarmes as $d)
                                                        {!! $d->obs_desarme_busqueda !!} <br>
                                                    @empty
                                                    @endforelse
                                                    <br>-->
                            ...
                            <table class="table table-bordered mt-2" id="invoice-item-table">
                                ...
                                <tbody id="invoice">
                                    @foreach ($invoice->invoice_items as $item)
                                        {{-- {{dd ($item->product->marcaModelo) }} --}}
                                        <tr id="product-{{ $item->item_id }}">
                                            {{-- <td>
                                            <b>@isset($item->product->item->allCar)
                                                {{ $item->product->vehiculo->company_id == 1 ? 'PM-': 'PC-' }}
                                                @else
                                                {{ $item->product->item->company_id == 1 ? 'PM-': 'PC-' }}
                                            @endisset{{ $item->item_id }}</b>
                                        </td> --}}
                                            <td>
                                    ...
                                    @if ($invoice->status != 'Paid')
                                        <tr>
                                            ...
                                            <td class="text-right">
                                                ...
                                                @endphp
                                                <span
                                                    style="{{ $amount_due < 0 ? 'color: #ff0000; font-weight: bold;' : '' }}">
                                                ...
                                                @if ($client_currency != $base_currency)
                                                    ...
                                                    @endphp
                                                    <span
                                                        style="{{ $converted_amount_due < 0 ? 'color: #ff0000; font-weight: bold;' : '' }}">
							...
							<div class="table-responsive">
                                <table class="table table-bordered" id="invoice-payment-history-table">
                                    <thead class="base_color">
                                        ...
                                        <tr>
											...
											<th>{{ _lang('Productos') }}</th>
												{{-- <th>{{ _lang('Tax') }}</th>
												<th class="text-right">{{ _lang('Saldo a Favor').' '.$currency }}</th> --}}
                                        </tr>
                                    </thead>
                                    <tbody>
									@php $total=0; @endphp
                                        @foreach ($salesReturns as $dev)
                                            <tr id="transaction-{{ $dev->id }}">
                                                ...
                                                {{-- <td class="text-right">
                                                    {{ decimalPlace($dev->tax_amount, currency($dev->account->account_currency)) }}
                                                </td>
                                                <td class="text-right">
                                                    {{ decimalPlace($dev->grand_total, currency($dev->account->account_currency)) }}
                                                </td> --}}
                                            </tr>
                                        @endforeach
                                    </tbody>
									{{-- <tfoot class="tfoot active">
										<tr>
											<th colspan="4">{{ _lang('Total') }}</th>
											<th class="text-right" id="total-tax">0.00</th>
											<th class="text-right" id="total">{{ decimalPlace($total, currency($dev->account->account_currency)) }}</th>
										</tr>
									</tfoot> --}}
                                </table>
                            </div>
