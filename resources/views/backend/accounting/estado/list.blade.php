...
<div class="row">
	<div class="col-lg-12">
		...
		<a class="btn btn-primary btn-xs ajax-modal" data-title="{{ _lang('Create') }}" href="{{ route('estado.create')
		...
		<div class="card mt-2">
			...
			<div class="card-body">
				<table id="roles_table" class="table table-bordered data-table">
					...
					<tbody>
					    @foreach($estados as $m_m)
					    <tr data-id="row_{{ $m_m->id }}">
							<td class='name'>{{ $m_m->id }}</td>
							<td class='name'>{{ $m_m->estado }}</td>
							...
							<td class="text-center">
								<div class="dropdown">
								  ...
								  <form action="{{ action('EstadoController@destroy', $m_m['id']) }}" method="post">
									{{ csrf_field() }}
									<input name="_method" type="hidden" value="DELETE">
									
									<div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
										<a href="{{ action('EstadoController@edit', $m_m['id']) }}" data-title="{{ _lang
										('Update') }}" class="dropdown-item ajax-modal"><i class="mdi
										mdi-pencil"></i> {{ _lang('Edit') }}</a>
										<a href="{{ action('EstadoController@show', $m_m['id']) }}" data-title="{{ _lang
