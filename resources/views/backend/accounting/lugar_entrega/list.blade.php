...
<div class="row">
	<div class="col-lg-12">
		...
		<a class="btn btn-primary btn-xs ajax-modal" data-title="{{ _lang('Create') }}" href="{{ route('lugarentrega.create')
								...
								<div class="dropdown">
								  ...
								  <form action="{{ action('LugarEntregaController@destroy', $m_m['id']) }}" method="post">
									...
									<div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
										<a href="{{ action('LugarEntregaController@edit', $m_m['id']) }}" data-title="{{ _lang
										('Update') }}" class="dropdown-item ajax-modal"><i class="mdi
										mdi-pencil"></i> {{ _lang('Edit') }}</a>
										<a href="{{ action('LugarEntregaController@show', $m_m['id']) }}" data-title="{{ _lang
