                                ...
                                <tbody>
                                    @foreach ($invoices as $invoice)
                                        ...
                                        <tr id="row_{{ $invoice->id }}">
                                            ...
                                            <td class='due_date'>{!! $html !!}</td>
                                            <td class='grand_total text-right'>
                                                {{ decimalPlace($invoice->grand_total-$paid_dev, $currency) }}</td>
                                            <td class='paid text-right'>{{ decimalPlace($paid, $currency) }}</td>

                                            <td class='paid text-right'
                                                style="{{ $invoice->grand_total - $paid < 0 ? 'color: #ff0000; font-weight: bold;' : '' }}">
                                                {{ decimalPlace($invoice->grand_total - ($paid+$paid_dev), $currency) }}</td>
                            ...
                            <table id='summaries_table' class="table">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-center">{{ _lang('Status') }}</th>
                                        <th class="text-right">{{ _lang('Acumulado') }}</th>
                                        {{-- <th class="text-center">{{ _lang('Action') }}</th> --}}
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    @foreach ($summaries as $sumary)
                                        ...
                                        <tr id="row_{{ $sumary->id }}">
                                            @php
                                        $html="";
                                            $paid_dev = 0;
                                            $product_return_ = DB::select("select invoices.id,invoices.invoice_number,invoice_items.product_id,products_returns.product_id as productoid, invoice_items.sub_total from `invoices` inner join `invoice_items` on `invoice_items`.`invoice_id` = `invoices`.`id` left join `products_returns` on products_returns.invoice_id=invoices.id and  products_returns.product_id=invoice_items.product_id AND products_returns.status='procesada' WHERE `invoices`.`related_to` = 'contacts' AND invoices.id IN ($sumary->id)
                                            GROUP BY invoices.id,invoices.invoice_number,invoice_items.product_id");
                                            ...
                                            $acumulado =$acumulado+ ($sumary->grand_total - ($sumary->cobrado));


                                           @endphp
                                            <td><input type="checkbox" class="row-checkbox"></td>
                                            <!-- Checkbox por fila -->
                                            <td class='invoice_number'>{{ $sumary->number }}</td>
                                            <td class='due_date'>
                                                {{ date($date_format, strtotime($sumary->date)) }}</td>
                                            <td class=''>{{$sumary->tipo}} {!! $html !!}</td>
                                            <td class='grand_total text-right' style="{{ $sumary->grand_total < 0 ? 'color: #ff0000; font-weight: bold;' : '' }}">
                                                {{ decimalPlace($sumary->grand_total-$paid_dev, $currency) }}</td>
                                            <td class='paid text-right'>{{ decimalPlace($sumary->cobrado, $currency) }}</td>

                                            <td data-saldo="{{ ($sumary->grand_total - ($sumary->cobrado+$paid_dev)) }}" class='paid text-right'
                                                style="{{ $sumary->grand_total - $sumary->cobrado < 0 ? 'color: #ff0000; font-weight: bold;' : '' }}">
                                                {{ decimalPlace($sumary->grand_total - ($sumary->cobrado+$paid_dev), $currency) }}</td>
                                            
                                            

                                            <td class='status text-center'>{!! strip_tags(invoice_status($sumary->status), '<span>') !!}</td>

                                            <td class='paid text-right'>{{ decimalPlace($acumulado, $currency) }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                                <tfoot align="right">
                            ...
                            <table id='sales_return_table' class="table table-bordered">
                                <thead>
                                    <tr>
                                        ...
                                        <th>{{ _lang('Date') }}</th>
                                        <th>{{ _lang('Productos Devueltos') }}</th>
                                        <th class="text-right">{{ _lang('Importe') }}</th>
                                        <th class="text-center">{{ _lang('Action') }}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    @foreach ($sales_returns as $row)
                                        <tr id="row_{{ $row->id }}">
                                            <td class='invoice_number'>{{ $row->id }}</td>

                                            <td class='due_date'>
                                                {{ date($date_format, strtotime($row->return_date)) }}</td>
                                            <td class='detalle'>
											 @php $items = ''; @endphp
                                                @foreach ($row->sales_return_items as $item)
                                                    @php $items.= "(" . (($item->company_id == 1) ? 'PM-' : 'PC-') . $item->product_id . ") "   .  $item->item->item_name . ';' @endphp
                                                @endforeach
                                                {{ $items }}
                                            </td>
                                            <td class='grand_total text-right'>
                                                {{ decimalPlace($row->grand_total, $currency) }}
                                            </td>
                                            <td class="text-center">
											     <div class="dropdown">
                                                    <button class="btn btn-primary btn-xs dropdown-toggle" type="button"
        ...
        $(document).ready(function() {
            ...
            var summaries_table = $('#summaries_table').DataTable({
                ...
                var api = this.api();
                    ...
                    function add(a, b) {
                        return 1 * parseFloat(a).toFixed(2) + 1 * parseFloat(b).toFixed(2);
                    }

                    //Total over all pages
                    total = api.column(6).nodes().toArray().map(function(node) {
                        return $(node).attr('data-saldo');
                    });

                     var total_general = total.reduce(add, 0);

                     //Total over all pages
                    paget = api.column(6, { page: 'current' }).nodes().toArray().map(function(node) {
                        return $(node).attr('data-saldo');
                    });

                    //.toFixed(2)
                   
                     var pageTotal = paget.reduce(add, 0);
                     $(api.column(5).footer()).html('Total General {!!$currency!!}: '+  FormatNumber(total_general,2));
                     $(api.column(6).footer()).html(" {!!$currency!!} " + FormatNumber(pageTotal,2));
