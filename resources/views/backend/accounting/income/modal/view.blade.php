...
<div class="card">
	<div class="card-body">
	  <table class="table table-bordered">
		...
		<tr><td>{{ _lang('Payer Income Expense') }}</td><td>{{ isset($transaction->payer->contact_name) ? $transaction->payer->contact_name : '' }}</td></tr>
			...
			<td>{{ _lang('Attachment') }}</td>
				<td>
				  @if($transaction->attachment != "")
				   <a href="{{ asset('public/uploads/transactions/'.urlencode($transaction->attachment)) }}" target="_blank" class="btn btn-primary btn-xs">{{ _lang('View Attachment') }}</a>
				  @else
