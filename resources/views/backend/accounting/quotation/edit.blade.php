			...
			<div class="card-body">
				<form method="post" class="validate" autocomplete="off" action="{{ action('QuotationController@update', $id) }}" enctype="multipart/form-data">
					...
					<div class="row">
						...
						<select class="form-control d-none" name="template">
							...
							<div class="col-lg-3 mb-2">
								...
								<select class="form-control select2 select-filter" name="vendedor">
									...
									{{ create_option('users','id',['name','email'],$quotation->user_id,array('role_id=' => $rol)) }}
								</select>
							</div>
						...
						<div class="col-md-3 {{ $quotation->related_to == 'contacts' ? '' : 'd-none' }}" id="contacts">
							<div class="form-group">
								...
								<select class="form-control select2-ajax" data-value="id" data-display="contact_name" data-display2="dni_cuit" data-table="contacts" data-where=""  data-table="contacts"  name="client_id" id="client_id">
									<option value="">{{ _lang('Select One') }}</option>
									{{-- create_option("contacts","id","contact_name", $quotation->related_id, array("company_id="=>company_id())) --}}
									{{ create_option("contacts","id","contact_name",$quotation->related_id) }}
								</select>
							</div>
						</div>
					...
					<div class="form-group select-product-container">
							...
							<div class="col-md-6">
								...
								('Add Product') }}" class="ajax-modal select2-add"><i class="ti-plus"></i> {{ _lang('Add New') }}</a>
									...
									<select class="form-control" data-value="products.id" data-display="item_name"
											data-table="items" data-where="9" data-option = "{{isset($idCar) ? "products
							...
							<div class="col-md-6">
								<div class="form-group">
									...
									data-display2="marcas.marca" data-display3="modelos.modelo"
										...
										<option {{old('car_id',$quotation->car_id ?? '') == $vehiculo->id? 'selected' :''}} value="{{
										...
										{{-- @forelse($vehiculos as $v)
											<option {{old('car_id',$quotation->car_id ?? '') == $v->id? 'selected' :''}} value="{{
										$v->id}}">{{
						...
						<div class="col-md-6 d-none">
							<div class="form-group">
								...
								<select class="form-control "  name="desarmar" id="desarmar">
									<option {{$quotation->desarmar == 0 ? 'selected' : ''}} value="0">{{ _lang('No')
									}}</option>
									<option {{$quotation->desarmar == 1 ? 'selected' : ''}} value="1">{{ _lang('Si')
									}}</option>
								</select>
							</div>
						</div>
						...
						{{-- <div class="col-md-6">
							<div class="form-group">
								...
								<input type="date" class="form-control" name="fecha_entrega" value="{{$quotation->fecha_entrega}}">
							</div>
						</div>
						...
						<div class="col-md-6">
							<div class="form-group">
								...
								<select class="form-control select2"  name="retiro" id="retiro">
									...
									<option {{$quotation->retiro == 1 ? 'selected' : ''}} value="1">{{ _lang
									('Retirado') }}</option>
									<option {{$quotation->retiro == 2 ? 'selected' : ''}} value="2">{{ _lang
									('Enviado') }}</option>
								</select>
							</div>
						</div>
						...
						<div class="col-md-6">
							<div class="form-group">
								...
								<select class="form-control select2 "  name="entregado_por"
									...
									@foreach($users as $u)
										<option {{$u->id == $quotation->entregado_por ? 'selected' : ''}}
												value="{{$u->id}}">{{
										$u->
										name}}</option>
									@endforeach
									...
									<tbody>
										@foreach($quotation->quotation_items as $item)
											<tr id="product-{{ $item->item_id }}">
												...
												<td>
													{{ $item->idCar }}
													<input type="hidden" name="autos[]" value="{{$item->idCar}}">
												</td>
								...
								<div class="form-group">
									...
									<select class="form-control" name="status" id="status">
										...
										@foreach($status as $key=>$s)
											<option {{$key == $quotation->status ? 'selected' :''}}
													value="{{$key}}">{{$s}}</option>
										@endforeach
