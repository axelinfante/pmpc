...
<div class="row">
    <div class="col-sm-3">
	    <ul class="nav flex-column nav-tabs settings-tab" role="tablist">
		  ...
		  <li class="nav-item"><a class="nav-link" data-toggle="tab" href="#privacy_policy">{{ _lang('Privacy Policy') }}</a></li>
	...
	<div class="col-sm-9">
		<div class="tab-content">
			...
			<div id="privacy_policy" class="tab-pane fade">
				<div class="card">
				    <div class="card-body">
						<h4 class="mb-4 header-title panel-title">{{ _lang('Privacy Policy Page') }}</h4>

						  <ul class="nav nav-tabs">
						    @foreach($language_list as $language)
							 	<li class="nav-item">
							 	   <a class="nav-link {{ $loop->first ? 'active' : '' }}" data-toggle="tab" href="#privacy-language-{{ $loop->index + 1 }}">{{ $language }}</a>
							  	</li>
						    @endforeach
						</ul>
						<br>
						...
						<form method="post" class="appsvan-submit params-panel" autocomplete="off" action="{{ url('administration/theme_option/update') }}" enctype="multipart/form-data">
							...
							<div class="tab-content">
								...
								@foreach($language_list as $language)
								<div class="tab-pane container {{ $loop->first ? 'active' : '' }}" id="privacy-language-{{ $loop->index + 1 }}">
									<div class="row">
										
										<div class="col-md-12">
										  <div class="form-group">
											<label class="control-label">{{ _lang('Privacy Policy Content') }}</label>						
											<textarea class="form-control summernote" rows="10" name="privacy_policy_content[{{$language}}]">{{ get_array_option('privacy_policy_content',$language) }}</textarea>
										  </div>
										</div>
