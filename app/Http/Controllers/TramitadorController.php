...
class TramitadorController extends Controller
{
            ...
            ->whereHas('role', function ($query) {
                $query->where(function ($q) {
                    $q->where('name', 'Tramitador')
                        ...
                        ->orWhere('name', 'Tramitadores con retiros');
                });
    ...
    public function get_table_data(Request $request)
    {
        ...
        return Datatables::eloquent($cars)
            ...
            ->editColumn('id', function ($car) {
                $valor_actual= nroInternoAlias($car->company_id, $car->tipo_vehiculo, $car->id);  
                return '<a class="" href="' . action('TramitadorController@seguimiento', $car['id']) . '">' . $valor_actual .  '</a>';
            })
            ...
            ->addColumn('estado_tramite', function ($car) {
                ...
                if ($max_tramite >= 4 && $max_tramite <= 10) return 'En Gestoria';
                if ($max_tramite >= 11 ) return 'Finalizado';
                return 'Pendiente';
            })
    ...
    public function seguimiento($id)
    {
        ...
        if ($max_tramite_completado >= 1 && $max_tramite_completado <= 3) {
            $statusC = 'En Proceso';
        } elseif ($max_tramite_completado >= 4 && $max_tramite_completado <= 10) {
            $statusC = 'En Gestoria';
        } elseif ($max_tramite_completado >= 11) {
            $statusC = 'Finalizado';
        } else {
            $statusC = 'Pendiente';
        }
        ...
        $max_tramite_iniciado = DB::table('vehiculos_checkpoints')
            ...
            if ($max_tramite_iniciado >= 1 && $max_tramite_iniciado <= 3) {
                $statusI = 'En Proceso';
            } elseif ($max_tramite_iniciado >= 4 && $max_tramite_iniciado <= 10) {
                $statusI = 'En Gestoria';
            } elseif ($max_tramite_iniciado >= 11) {
                $statusI = 'Finalizado';
            } else {
                $statusI = 'Pendiente';
            }
