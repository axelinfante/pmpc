	...
	*/
	...
	public function pay(Request $request)
    {
		...
		if( $data['amount'] == 0 ){
	        ...
	        $company->staff_limit = unserialize($package->staff_limit)[$request->package_type];
	        $company->contacts_limit = unserialize($package->contacts_limit)[$request->package_type];
	        $company->invoice_limit = unserialize($package->invoice_limit)[$request->package_type];
	        $company->quotation_limit = unserialize($package->quotation_limit)[$request->package_type];
	        $company->project_management_module = unserialize($package->project_management_module)[$request->package_type];
	        $company->recurring_transaction = unserialize($package->recurring_transaction)[$request->package_type];
	        $company->live_chat = unserialize($package->live_chat)[$request->package_type];
	        $company->file_manager = unserialize($package->file_manager)[$request->package_type];
	        $company->online_payment = unserialize($package->online_payment)[$request->package_type];
	        $company->inventory_module = unserialize($package->inventory_module)[$request->package_type];
	...
	public function paypal_ipn(Request $request)
	{
		...
		if( $amount >= $payment->amount){
	        ...
	        $company->staff_limit = unserialize($package->staff_limit)[$payment->package_type];
	        $company->contacts_limit = unserialize($package->contacts_limit)[$payment->package_type];
	        $company->invoice_limit = unserialize($package->invoice_limit)[$payment->package_type];
	        $company->quotation_limit = unserialize($package->quotation_limit)[$payment->package_type];
	        $company->project_management_module = unserialize($package->project_management_module)[$payment->package_type];
	        $company->recurring_transaction = unserialize($package->recurring_transaction)[$payment->package_type];
	        $company->live_chat = unserialize($package->live_chat)[$payment->package_type];
	        $company->file_manager = unserialize($package->file_manager)[$payment->package_type];
	        $company->online_payment = unserialize($package->online_payment)[$payment->package_type];
