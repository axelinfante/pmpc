...
class OrdenDespachoController extends Controller
{
  ...
  <tbody>
    ...
    public function update(Request $request, $id)
    {
       ...
       /* if ($request->filled('eliminar_imagen') && $request->eliminar_imagen == 1 && $orden->foto_guia) {
            $filePath = $path . '/' . $orden->foto_guia;
            if (file_exists($filePath)) {
                unlink($filePath);
            }
        ...
        /*if ($request->hasFile('imagen')) {
            ...
            if ($orden->foto_guia && file_exists($path . '/' . $orden->foto_guia)) {
                unlink($path . '/' . $orden->imagen);
            }
