...
class DashboardController extends Controller
{
    ...
    public function index()
    {
		...
		if( $type  == 'admin' ){
			...
			return view('backend/dashboard-'.$type,$data);
		}else if( ($type  == 'user' || $type  == 'staff') &&  strtolower(auth()->user()->role->name) != 'operario' &&
            strtolower(auth()->user()->role->name) != 'receptor' &&
            strtolower(auth()->user()->role->name) != 'gerente de operarios' &&
            strtolower(auth()->user()->role->name) != 'tramitador con retiros' && strtolower(auth()->user()->role->name) != 'retiros'  && strtolower(auth()->user()->role->name) != 'despacho' &&
            strtolower(auth()->user()->role->name) != 'tramitador' ){
			$company_id = company_id();
			$data = array();
			$data['current_month_income'] = current_month_income();
			$data['current_month_income_usd'] = current_month_income_usd();
    ...
    strtolower(auth()->user()->role->name) == 'gerencial' || 
    strtolower(auth()->user()->role->name) == 'transportista' || 
    strtolower(auth()->user()->role->name) == 'administrativo' || 
    strtolower(auth()->user()->role->name) == 'gerente de operarios' || 
    strtolower(auth()->user()->role->name) == 'facturador' || 
    strtolower(auth()->user()->role->name) == 'revendedor' || 
    strtolower(auth()->user()->role->name) == 'retiros' || 
    strtolower(auth()->user()->role->name) == 'cajera' || 
    strtolower(auth()->user()->role->name) == 'despacho' || 
    strtolower(auth()->user()->role->name) == 'receptor' || 
    strtolower(auth()->user()->role->name) == 'carga de productos' || 
    strtolower(auth()->user()->role->name) == 'pagos' || 
    strtolower(auth()->user()->role->name) == 'actualización de estados' || 
    strtolower(auth()->user()->role->name) == 'tramitador con gastos' || 
    strtolower(auth()->user()->role->name) == 'cadete' || 
    strtolower(auth()->user()->role->name) == 'carga de productos limitada') {
                // $data['total_invoice_count'] = \App\Invoice::where('company_id',$company_id)->where('user_id',auth()
                //     ->id())
                //     ->count();

                // $data['unpaid_invoice_count'] = \App\Invoice::where('status','Unpaid')->where('user_id',auth()
                //     ->id())
                //     ->where('company_id',$company_id)->count();

                // $data['canceled_invoice_count'] = \App\Invoice::where('status','Canceled')->where('user_id',auth()
                //     ->id())
                //     ->where('company_id',$company_id)->count();
                ...
                // $data['current_month_expense'] = 0;
				return view('backend/dashboard-vendedor',$data);
            }
