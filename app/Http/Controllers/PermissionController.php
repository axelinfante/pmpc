    ...
    public function index($role_id = '')
    {
		...
		foreach ($routeCollection as $route) {
			...
			if (array_key_exists('controller', $action)) {
				...
				if ($route->uri=="tramitadores/edit-caso") {
					$routes[$explodedAction[0]][] = array("method" => $explodedAction[1], "action" => $route->action);
				}
			}
		}
		...
		foreach($routes as $key => $route){
			foreach($route as $r){
				// Excluir métodos GET comunes pero incluir métodos importantes que comienzan con 'get'
				if (strpos($r['method'], 'get') === 0) {
					// Métodos importantes que SÍ deben tener control de permisos aunque comiencen con 'get'
					$importantGetMethods = array(
						"getContactos",
						"getMovimientos",
						"getResumen",
					);

					if (!in_array($r['method'], $importantGetMethods)) {
						continue;
					}
				}
		...
		,['grupo' => 'Procesos interno', 'activo'=>'Si', 'items' => ['pi_consulta_venta']],
		['grupo' => 'Traslado de productos', 'activo' => 'Si', 'items' => ['crear-trasladomercancia', 'editar-trasladomercancia', 'eliminar-trasladomercancia', 'ver-trasladomercancia']],
		]);

		$permisos=$this->collection;

		return view('backend.permission.create',compact('permission','permission_list','role_id','permisos'));
