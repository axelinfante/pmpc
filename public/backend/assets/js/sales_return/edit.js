	    ...
	    $.ajax({
	        ...
	        success: function(data) {
	            ...
	            var product_row = `<tr id="product-${item['id']}">
											...
											<td class="text-center quantity">1<input type="hidden" name="quantity[]" min="1" class="form-control input-quantity text-center" value="1"></td>
											...
											<input type="hidden" name="sales_return_items_id[]" value="0">
...
function update_summary(){
    ...
    $("#product_sub_total").html(_currency + ' ' + product_total.toFixed(2));
	...
	if(client_currency != ''){
		 convert_currency(grand_total+10);
	}else{
