...
(function($) {
	...
	*/
    ...
    $(document).on('click','.btn-remove-2-cancelar',function(){
        var link = $(this).attr('href');
        //Sweet Alert for delete action
        Swal.fire({
            title: $lang_alert_title,
            text: $lang_alert_message_cancelar_venta,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
	...
	$(document).on('click','.btn-remove-invoice',function(){
	  ...
	  input: 'text',
		  /*input: 'select',
		  inputOptions: {
			'quitar': 'Restaurar productos',
			'descompuesto': 'Productos descompuestos',
		  ...
		  cancelButtonText: $lang_cancel_button_text
			...
			//console.log(result);
			let html = '<input name="note" value="'+result.value+'">'
...
// debugger
	...
	if ($(".select2-ajax").length) {
		$('.select2-ajax').each(function(i, obj) {
			...
			$(this).select2({
			  ajax: {
				url: _url + '/ajax/get_table_data?table=' + $(this).data('table') + '&value=' + $(this).data('value') + '&display=' + $(this).data('display') + display2 + display3 + '&where=' +$(this).data('where')+ '&option=' +$(this).data('option')+'&company=' +$(this).data('company'),
    ...
    if($('#car_id').val()) {
        $.ajax({
            url :_url + '/vehiculo/get-company/'+$('#car_id').val(),
            async: false,
            dataType: 'json',
...
//                console.log(data.company);
	...
	$('.select2-ajax').on('change.select2',function (e) {
	...
	$(document).on("click",".ajax-modal",function(){
		 ...
		 $.ajax({
			 ...
			 // Selecciona el modal
				...
				if(fullscreen ==true){
	    // Elimina las clases de tamaño existentes
  			   modalDialog.removeClass('modal-lg');
					 modalDialog.addClass("fullscreen-modal modal-xl");
				}else{
					//$("#main_modal >.modal-dialog").removeClass("fullscreen-modal modal-xl");
					modalDialog.removeClass('fullscreen-modal modal-xl');
...
function init_datepicker(){
	...
	var fake_format = picker_date_format[date_format.indexOf("d/m/Y")];
